...
namespace RFDown.Comparison
{
    ...
    public static class FastCompare
    {
        ...
        public static async Task<bool> EqualsAsync(Stream left, Stream right)
        {
            ...
            {
                int leftRead;
                while ((leftRead = await left.ReadAsync(leftBuffer, 0, leftBuffer.Length)) > 0)
                {
                    int rightRead = await right.ReadAsync(rightBuffer, 0, rightBuffer.Length);
